...
- 本地 Agent 对话同步：新 Codex 任务加载 Infinite Canvas MCP 时，面板会自动绑定同工作区最近活动的 Codex 任务；需验证桌面端既有消息能在面板历史中显示、面板继续发送后仍写入同一任务、手动进入其他历史仍可切换，以及本地 HTTP Agent 未启动时 MCP 不会被阻塞。
- 画布复制图片：HTTPS 安全页面优先使用浏览器标准剪贴板 API；局域网 HTTP 页面在本机 Agent 已连接时通过受 token 保护的 Agent 端点把 PNG 写入 Windows 系统剪贴板，需验证有效 token 成功、无效或缺失 token 被拒绝、非 PNG 请求不会写入，以及未连接 Agent 时仍显示明确失败原因。
- 画布节点创建：鼠标左键双击画布空白区域会在点击位置打开节点选择菜单，可创建文本、图片、视频、音频、生成配置和组节点；双击已有节点或连线不应触发菜单。
- 本地 Agent 模型切换：对话栏动态展示当前 Codex 账号可用模型及其推理强度，切换任一选项后清空当前对话引用，首条消息发送时再用所选参数创建真实会话；需验证切换后不立即扫描空会话、不出现 rollout 路径读修复警告、“跟随 Codex”和各模型强度生效、刷新后选择保留以及旧对话不被改写。
- 本地 Agent NAS HTTP 部署：局域网 HTTP 来源缺少 `crypto.randomUUID` 时使用时间戳随机 ID 回退，需验证 Agent 面板可正常打开、连接和上传附件。
- 本地 Agent 空对话：新建对话或切换模型后，在发送首条消息前不读取尚未生成 rollout 的 thread，需验证控制台不再出现 `no rollout found`。
- 本地 Agent 图片上传：新增 `assets_add_file`，聊天附件会先通过 ffmpeg 解码并统一转成 Codex 支持的 PNG，再向 Codex 提供临时绝对路径；本地 PNG、JPEG、WebP、GIF、AVIF 图片可直接加入「我的素材」，需验证 30MB 限制、标题和标签写入、AVIF/HEIC 转码以及成功/失败后的临时文件清理。
- 本地 Agent Windows 网络盘：`X:` 工作空间自动使用兼容沙盒模式，本地磁盘仍使用 `workspace-write`；需验证网络盘可读写且网页“工具确认”保持开启。
- 本地 Agent 连接说明：新增提醒说明只有安装 Codex 插件或手动添加 MCP 后才会增加 Codex token 消耗，直接运行 `npx -y @kumuze/canvas-agent` 不会安装 MCP；需验证文案位置清晰。
...
- 全站 Agent：Codex 会话改为单一站点级连续线程（不再按 canvasId 分），跨页面/跨画布保持同一条对话上下文，历史记录即这条线程；本地 Agent workspace 从按画布分改为固定 `site` workspace（`~/.infinite-canvas/codex-workspaces/site`），已装 `@kumuze/canvas-agent` 的用户需更新到新版本才生效。

- npm 发布：验证 `@kumuze/canvas-agent@0.2.1` 可通过 `npx` 启动，并确认后续版本可由 `publish-canvas-agent.yml` 使用 npm Trusted Publishing 发布。
