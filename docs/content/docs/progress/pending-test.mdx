...
- 本地 Agent 模型切换：对话栏动态展示当前 Codex 账号可用模型及其推理强度，切换任一选项后保留当前对话与消息，并从下一轮开始使用所选参数；需验证切换时消息不消失、不出现 rollout 路径读修复警告、“跟随 Codex”和各模型强度生效、刷新后选择保留。
- 本地 Agent NAS HTTP 部署：局域网 HTTP 来源缺少 `crypto.randomUUID` 时使用时间戳随机 ID 回退，需验证 Agent 面板可正常打开、连接和上传附件。
- 本地 Agent 空对话：手动新建对话后，在发送首条消息前不读取尚未生成 rollout 的 thread，需验证控制台不再出现 `no rollout found`。
