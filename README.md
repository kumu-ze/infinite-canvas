...
- `web/`：Vite + React 前端，负责画布、工作台、配置和浏览器本地数据。
- `canvas-agent/`：可选的本地 Agent 服务，负责连接 Codex / Claude Code，并把 MCP 工具转发给当前网页。
- `plugins/infinite-canvas/`：Codex App 插件，安装后可从 Codex 直接打开画布并注册 MCP。
- `docs/`：功能、部署、画布数据结构和开发说明。

只使用画布和生图功能时，运行 `web/` 即可；需要网页侧边栏 Agent、Codex 或 Claude Code 时，再运行 `canvas-agent/`。

## 快速开始

AI API Key、Base URL、画布、素材和生成记录默认保存在浏览器本地。项目本身不提供公共 API Key，也不会替你托管模型服务。
...
开发服务器默认运行在 `http://localhost:3000`。首次打开后，进入右上角配置，填写自己的 OpenAI 兼容 `Base URL`、API Key 和默认模型。
...
运行后访问 `http://localhost:3000`。生产镜像使用 nginx 托管前端静态文件，并通过 `nginx.conf` 提供单页路由回退。

如果已经在其他环境生成了 `web/dist`，可以使用轻量运行镜像：

```bash
docker build -f Dockerfile.runtime -t infinite-canvas:runtime .
docker run --rm -p 3000:3000 infinite-canvas:runtime
```
...
Agent 默认只监听 `127.0.0.1:17371`。在网页的 Agent 面板中连接 Local URL 和 Connect token；也可以先检查：

```bash
curl http://127.0.0.1:17371/health
curl http://127.0.0.1:17371/config
```

网页侧边栏通过 Agent 连接 Codex / Claude Code。Agent 本身不会替你安装 Codex MCP；只有安装插件或手动添加 MCP 后，`infinite-canvas` 工具才会进入 Codex 上下文。

### Codex App 插件

插件源码位于 [`plugins/infinite-canvas/`](plugins/infinite-canvas)。详细安装和本地开发步骤见 [Codex App 插件文档](docs/content/docs/overview/codex-app-plugin.mdx)。不使用插件时，也可以直接运行 Agent，再在网页中手动连接。

### NAS 或反向代理部署

NAS 部署通常包含两个服务：前端静态站点和 `canvas-agent`。请根据自己的网络修改端口、`PUBLIC_URL`、反向代理和 Agent 的允许来源；不要把实际 Connect token、API Key、`env_file` 或持久卷路径提交到公开仓库。仓库中的 [`canvas-agent/Dockerfile.nas`](canvas-agent/Dockerfile.nas) 和额外的 nginx 代理规则属于部署模板，复用前请检查其中的局域网地址和代理目标。

更完整的 Docker 说明见 [Docker 部署](docs/content/docs/overview/docker.mdx)，Agent 的连接边界见 [本地 Codex 画布](docs/content/docs/overview/local-codex-canvas.mdx)。
